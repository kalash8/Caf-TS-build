    ...
    >
      <div className="flex justify-center w-full md:w-auto md:ml-8">
        <Link href="/vendor" className="flex items-center space-x-2">
          ...
          <span className="text-xl font-bold mt-1"> | Cafe Vendor</span>
        </Link>
      </div>
      {token && (
        ...
        <>
          ...
          <div className={`md:hidden absolute top-14 left-0 right-0 bg-[#003D5D] shadow-md rounded-b-xl ${isOpen ? 'block' : 'hidden'}`}>
            <div className="flex flex-col items-center py-4">
              <Link href="/vendor" className="py-2 hover:underline" onClick={() => setIsOpen(false)}>Menu</Link>
              <Link href="/vendor/orders" className="py-2 hover:underline" onClick={() => setIsOpen(false)}>Orders</Link>
